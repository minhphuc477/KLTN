...
class ZeldaGUI:
        ...
        if self.feature_flags.get('parallel_search', False):
            ...
            try:
                ...
                with mp.get_context('spawn').Pool(processes=4) as pool:
                    async_res = pool.map_async(worker_call, [0,1,2,3])
                    try:
                        timeout_s = int(os.environ.get('KLTN_PARALLEL_SEARCH_TIMEOUT', '10'))
                    except Exception:
                        timeout_s = 10
                    try:
                        results = async_res.get(timeout=timeout_s)
                    except Exception as e:
                        logger.warning("Parallel search pool timed out or failed: %r", e)
                        results = []
    ...
    def _render(self):
        ...
        # Diagnostic logging to help explain why nothing is drawn
        try:
            ...
            if start_r < h and start_c < w:
                ...
                sample_tile = None
            logger.info("Render sample tile at start: %r, images_contains=%r", sample_tile, sample_tile in getattr(self, 'images', {}))
        except Exception:
        ...
        # --- Permanent debug overlay (visible even when map appears black) ---
        try:
            # Small test rectangle to confirm blitting works
            pygame.draw.rect(self.screen, (255, 0, 255), (8, 8, 32, 32))

            dbg_x = 50
            dbg_y = 8
            small = self.small_font if hasattr(self, 'small_font') else pygame.font.SysFont('Arial', 12)
            env_map = (getattr(self.env,'width',0), getattr(self.env,'height',0)) if self.env is not None else (0,0)
            lines = [
                f"tiles_drawn={tiles_drawn}",
                f"TILE_SIZE={self.TILE_SIZE}",
                f"view_offset=({int(self.view_offset_x)},{int(self.view_offset_y)})",
                f"map={env_map[0]}x{env_map[1]}",
                f"images_count={len(getattr(self,'images',{}))} floor_in_images={SEMANTIC_PALETTE['FLOOR'] in getattr(self,'images',{})}",
            ]
            ...
            self.screen.blit(bg, (dbg_x - 8, dbg_y - 8))
            for i, ln in enumerate(lines):
                surf = small.render(ln, True, (220, 220, 220))
                self.screen.blit(surf, (dbg_x, dbg_y + i * 18))

            # If no tiles drawn, also draw a prominent center box and save a screenshot once
            if tiles_drawn == 0:
                center_box = pygame.Rect((self.screen_w//2 - 100, self.screen_h//2 - 100, 200, 200))
                pygame.draw.rect(self.screen, (255, 200, 0), center_box)
                try:
                    if not getattr(self, '_saved_debug_screenshot', False):
                        fname = f"zave_debug_{int(time.time())}.png"
                        pygame.image.save(self.screen, fname)
                        logger.info("Saved debug screenshot to %s", fname)
                        self._saved_debug_screenshot = True
                except Exception:
