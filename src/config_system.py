...
CONFIG_FIELDS: List[ConfigField] = [
    ...
    ConfigField("diffusion.warm_start_checkpoint", str, None, "Optional weights-only diffusion checkpoint. Resets epoch, optimizer, scheduler, and AMP scaler state.", allow_none=True),
    ...
    ConfigField("diffusion.latent_scale_factor", float, 1.0, "Fixed multiplier from raw VQ-VAE latents to diffusion space. Use a corpus-calibrated reciprocal standard deviation for the scaled-latent ablation; 1.0 preserves legacy checkpoints.", min_value=1e-8),
    ...
    ConfigField("diffusion.condition_strict_schema", bool, True, "Fail on graph-conditioning schema shape drift instead of pad/truncate compatibility alignment. Set false only for explicit legacy-checkpoint migration."),
    ...
    ConfigField("diffusion.puzzle_structure_dropout_prob", float, 0.0, "Experimental only: a solver-validated puzzle-on/puzzle-off counterfactual rate. Disabled by default because removing BLOCK tiles without recompiling the stage plan creates contradictory supervision.", min_value=0.0, max_value=1.0),
    ...
    ConfigField("diffusion.logic_initial_temperature", float, 1.0, "Initial LogicNet soft-planning temperature.", min_value=1e-6),
    ConfigField("diffusion.logic_final_temperature", float, 0.05, "Final LogicNet soft-planning temperature.", min_value=1e-6),
    ...
    ConfigField(
        ...
        str,
        "spatial",
        "Grammar rule-space for Block I. Use spatial for end-to-end maps; full is graph-only.",
        choices=("core", "spatial", "full"),
    ),
    ...
    ConfigField("generation.map_elites_preference_buffer_size", int, 0, "Bounded same-condition room candidate replay for QD-to-DPO export; zero disables it.", min_value=0),
    ...
    ConfigField("generation.end_to_end_validation_mode", str, "reject", "Final artifact contract: off, report staged evidence, or reject any failed/indeterminate applicable hard-validation stage.", choices=("off", "report", "reject")),
    ...
    ConfigField("fast_sampler.validate_ema_target", bool, True, "Use the EMA target network for fast-sampler validation. Disable only as an explicit student-versus-EMA ablation."),
    ...
    ConfigField("fast_sampler.puzzle_structure_dropout_prob", float, 0.0, "Experimental only: a solver-validated puzzle-on/puzzle-off counterfactual rate. Disabled by default because removing BLOCK tiles without recompiling the stage plan creates contradictory supervision.", min_value=0.0, max_value=1.0),
    ...
    ConfigField("fast_sampler.best_checkpoint_metric", str, "val_decode_ce_loss", "Metric used to select the best fast-sampler checkpoint. Falls back to train_loss when no validation split exists.", choices=("val_loss", "val_decode_ce_loss", "val_topology_decode_ce_loss", "val_puzzle_stage_semantic_loss", "train_loss")),
    ...
    ConfigField("masked_room.condition_strict_schema", bool, True, "Fail on masked-room graph-conditioning schema shape drift instead of pad/truncate compatibility alignment. Set false only for explicit legacy-checkpoint migration."),
    ...
    ConfigField("masked_room.logic_initial_temperature", float, 1.0, "Initial masked-room LogicNet soft-planning temperature.", min_value=1e-6),
    ConfigField("masked_room.logic_final_temperature", float, 0.05, "Final masked-room LogicNet soft-planning temperature.", min_value=1e-6),
    ConfigField("masked_room.puzzle_structure_dropout_prob", float, 0.0, "Experimental only: a solver-validated puzzle-on/puzzle-off counterfactual rate. Disabled by default because removing BLOCK tiles without recompiling the stage plan creates contradictory supervision.", min_value=0.0, max_value=1.0),
