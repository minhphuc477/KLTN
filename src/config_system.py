...
CONFIG_FIELDS: List[ConfigField] = [
    ...
    ConfigField("generation.fast_sampler_teacher_fallback_enabled", bool, True, "Allow fast-sampler rooms to fall back to the full diffusion teacher when runtime quality guards trigger."),
    ConfigField("generation.masked_room_teacher_fallback_enabled", bool, True, "Allow masked-room outputs with obvious structural noise to fall back to the diffusion teacher during runtime generation."),
    ...
    ConfigField("fast_sampler.best_checkpoint_metric", str, "val_decode_ce_loss", "Metric used to select the best fast-sampler checkpoint. Falls back to train_loss when no validation split exists.", choices=("val_loss", "val_decode_ce_loss", "train_loss")),
