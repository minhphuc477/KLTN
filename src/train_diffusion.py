...
from src.training.diffusion_config import (
    DiffusionTrainingConfig as DiffusionTrainingConfig,
    build_diffusion_training_config_from_args as build_diffusion_training_config_from_args,
    diffusion_training_kwargs_from_resolved_config as diffusion_training_kwargs_from_resolved_config,
)
...
class DiffusionTrainer:
    ...
    def _create_logic_net(self) -> LogicNet:
        ...
        return LogicNet(
            ...
            graph_pathfinder_type=self.config.logic_graph_pathfinder,
    ...
    def _latent_cache_key(self, x: torch.Tensor) -> Optional[Tuple[Any, ...]]:
        ...
        current_version = FrozenLatentCache.module_version_token(self.vqvae)
        if current_version != getattr(self, "_vqvae_cache_version", None):
            cache = getattr(self, "_latent_cache", None)
            if isinstance(cache, FrozenLatentCache):
                cache.clear()
            self._vqvae_cache_fingerprint = FrozenLatentCache.module_state_fingerprint(self.vqvae)
            self._vqvae_cache_version = current_version
        namespace = (
            ...
            str(self._vqvae_cache_fingerprint),
        )
    ...
    def _stack_diffusion_graph_batch(self, graph_list: List[dict]) -> Optional[Dict[str, torch.Tensor]]:
        ...
        topo_maps = []
        logic_mask_maps: Dict[str, List[torch.Tensor]] = {
            "logic_source_mask": [],
            "logic_target_mask": [],
        }
    ...
    def _try_stack_dungeon_scope_graph_batch(self, graph_list: List[dict]) -> Optional[Dict[str, torch.Tensor]]:
        ...
        for mask_name, mask_rows in logic_mask_rows.items():
            if len(mask_rows) == len(graph_list):
                batch_graph[mask_name] = torch.cat(mask_rows, dim=0)
        return batch_graph
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help='Mission-graph Bellman backend; sparse mode scales with real graph edges.',
    )
    parser.add_argument(
