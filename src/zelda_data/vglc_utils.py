...
from src.utils.graph_utils import validate_goal_subgraph as canonical_validate_goal_subgraph
...
class VGLCTopologyValidator:
    ...
    def validate_goal_subgraph(graph: nx.Graph) -> Tuple[bool, str]:
        ...
        """
        # Delegate to canonical validator to keep VGLC and pipeline checks aligned.
        is_valid, errors = canonical_validate_goal_subgraph(graph)
        if is_valid:
            return True, "Valid goal subgraph"
        if not errors:
            return False, "Goal subgraph validation failed"
