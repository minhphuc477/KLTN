...
def load_assets(*, gui, semantic_palette, pygame, default_tile_color_map_helper, build_tile_images_helper, build_stair_marker_sprite_helper):
    """Build base tile assets, link sprite, and optional stair marker sprite."""
    color_map = default_tile_color_map_helper(semantic_palette=semantic_palette)
    gui.images = build_tile_images_helper(
    ...
    try:
        gui.stair_sprite, gui.stair_anim_phase = build_stair_marker_sprite_helper(
            tile_size=gui.TILE_SIZE,
            pygame=pygame,
        )
    except Exception:
...
def create_link_sprite(*, tile_size, pygame, build_link_sprite_helper):
    """Create the link sprite surface for the current tile size."""
    return build_link_sprite_helper(tile_size=tile_size, pygame=pygame)
