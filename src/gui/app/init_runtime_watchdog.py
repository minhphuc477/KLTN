...
def initialize_runtime_timing_state(*, gui: Any, pygame: Any, os_module: Any, time_module: Any, threading_module: Any, logger: Any) -> None:
    ...
    gui._display_check_interval = float(os_module.environ.get("KLTN_DISPLAY_CHECK_INTERVAL", "1.0"))
    gui._display_check_last = time_module.time()
    gui._display_recovery_attempts = 0
    gui._display_recovery_attempts_limit = int(os_module.environ.get("KLTN_DISPLAY_RECOVER_LIMIT", "3"))
    ...
    gui.debug_panel_click_padding = int(os_module.environ.get("KLTN_DEBUG_PANEL_PADDING", "40")) if gui.debug_control_panel else 0
    ...
    try:
        ...
        gui._watchdog_threshold = float(os_module.environ.get("KLTN_WATCHDOG_THRESHOLD", "1.25"))
        gui._watchdog_last_dump = 0.0
        gui._watchdog_dump_limit = int(os_module.environ.get("KLTN_WATCHDOG_DUMP_LIMIT", "3"))
    ...
    try:
        gui._empty_frame_recovery_threshold = int(os_module.environ.get("KLTN_EMPTY_FRAME_RECOVERY", "8"))
    except Exception:
