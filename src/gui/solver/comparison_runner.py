...
def run_solver_comparison(
    ...
    def _worker():
        ...
        def _fallback_used_from_meta(meta: dict[str, Any]) -> bool:
            if not isinstance(meta, dict):
                return False
            if bool(meta.get("used_fallback", False)):
                return True
            algo_label = str(meta.get("algorithm", "")).strip().lower()
            return "fallback" in algo_label
        ...
        for _idx, name in enumerate(alg_names):
            ...
            if name == "StateSpace":
                try:
                    if not (hasattr(gui.env, "original_grid") or hasattr(gui.env, "grid")):
                        raise RuntimeError("State-space solver unavailable for current env")
                    try:
            ...
            if name in set(GUI_ALGORITHM_NAMES[:7]):
                try:
                    ...
                    alg_idx = GUI_ALGORITHM_NAMES.index(name)
                    ...
                    priority_options = {
                        ...
                        "allow_diagonals": bool(gui.feature_flags.get("diagonal_movement", False)),
                    }
                    ...
                    fallback_used = _fallback_used_from_meta(solver_meta)
